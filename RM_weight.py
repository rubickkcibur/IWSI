...
def RM_weight():
    ...
    logger.info('Initializing RM model...')
    modelL = transformers.AutoModelForSequenceClassification.from_pretrained(reward_name, torch_dtype=torch.bfloat16)
    tokenizerL = transformers.AutoTokenizer.from_pretrained(reward_name, padding_side = "right")
    ...
    prompts = [x["question"] + tokenizerL.eos_token + x["answer"] for x in train_dataset]
    dump_file_name = "RM_weight.json"

    tokenizerL.padding_side="right"
    ...
    with accelerator.split_between_processes(prompts) as prompts:
        ...
        for prompts_tokenized in prompt_batches:
            ...
            with torch.no_grad():
                score = modelL(**prompts_tokenized).logits.cpu().detach()
            scores.extend(score)
            if accelerator.is_local_main_process:
                pbar.update(1)

        scores = [ scores ]
    scores_gathered=gather_object(scores)

    if accelerator.is_main_process:
        total_results = []
        for r in scores_gathered:
            total_results += r
